...
class FeatureEngineering:
    ...
    def run(self):
        ...
        X_train, y_train = self.build_sequences(train_split, feature_cols)

        logging.info("Building validation sequences...")
        X_val, y_val = self.build_sequences(val_split, feature_cols)
        ...
        for file in [self.config.X_train, self.config.y_train,
